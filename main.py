...
class MyPlugin(Star):
    ...
    async def ban_user(self , user_id : str , platform : str , times : pendulum.Duration) ->tuple:
        self.ban_list = self.get_ban_list()
        ...
        if user_id in self.ban_list['banners'][platform] :
            if self.ban_list['banners'][platform][user_id] <= time.time() :
                ...
                time_str = datetime.datetime.fromtimestamp(self.ban_list['banners'][platform][user_id]).strftime('%Y年%m月%d日 %H:%M:%S')
                return "Fail",f"用户正在封禁中，预计解封时间{time_str}，请尝试解封后再试"
    ...
    async def sf_ban(self , event:AstrMessageEvent  , user_id : str , times : str ,plat_name : str|None = None) :
        """按照id封禁某位用户一段时间"""
        self.ban_list = self.get_ban_list()
        ...
        if not plat_name in self.ban_list['available_platforms'] :
            chain = MessageChain().message(f"消息平台{plat_name}不存在。目前可用的消息平台{self.ban_list['available_platforms']}，详情请联系管理员")
        ...
        try :
            ...
            chain = MessageChain().message(f"这不是一个符合ISO8601规范的时间持续时间，请在核实后重试")
            await event.send(chain)
            return
        state , detail = await self.ban_user(user_id , plat_name , ban_time)
        if state == 'Success' :
            chain = MessageChain().message(f"用户{user_id}封禁成功，预计解封时间{detail}")
        else :
    ...
    async def sf_unban(self , event:AstrMessageEvent , user_id :str , plat_name : str|None = None) :
        """按照id手动解封某位用户"""
        self.ban_list = self.get_ban_list()
        ...
        if plat_name not in self.ban_list['available_platforms'] :
            chain = MessageChain().message(f"所选消息平台{plat_name}不存在，请核实后重试")
            await event.send(chain)
            return
    ...
    async def sf_bancount(self , event:AstrMessageEvent , count : int , times : str , plat_name:str|None = None) :
        """按照封禁次数自动封禁一批用户一段时间"""
        self.ban_list = self.get_ban_list()
        config = self.context.get_config(event.unified_msg_origin)
        ...
        if plat_name is not None and plat_name not in self.ban_list['available_platforms'] :
            chain = MessageChain().message(f"所选消息平台{plat_name}不存在，请核实后重试")
            await event.send(chain)
            return
    ...
    async def sf_check(self , event:AstrMessageEvent , plat_name : str|None = None) :
        ...
        if not event.get_sender_id() in config['admins_id'] :
            chain = MessageChain().message(f"该命令仅管理员有权使用，您不是管理员，无法使用")
            await event.send(chain)
            return
    ...
    async def sf_clear(self , event:AstrMessageEvent , user_id:str , plat_name : str|None = None) :
        """手动清理特定用户的所有违规记录"""
        config = self.context.get_config(event.unified_msg_origin)
        self.ban_list = self.get_ban_list()
    ...
    async def check_request(self, event: AstrMessageEvent , req : ProviderRequest):
        ...
        if sender_plat in self.ban_list['available_platforms'] and sender_id in self.ban_list['banners'][sender_plat] :
            if time.time() >= self.ban_list['banners'][sender_plat][sender_id] :
                ...
                except_time = datetime.datetime.fromtimestamp(times)
                chain = MessageChain().message(f"你在被封禁中，具体情况请联系管理员。预计解封时间:{except_time.strftime('%Y年%m月%d日 %H:%M:%S')}")
                await event.send(chain)
                event.stop_event()
                return

        if sender_plat in self.ban_list['available_platforms'] and self.ban_list['prohibits'][sender_plat].get(sender_id , None) is not None :
            self.ban_list['prohibits'][sender_plat].pop(sender_id)

        self.write_ban(self.ban_list)
      
        system_prompt = (await self.context.persona_manager.get_persona(self.config['filter_prompt'])).system_prompt
        ...
        try :
            filter_res = await self.context.llm_generate(
                chat_provider_id= self.config['filter_config'],
                contexts=msg
            )
        ...
        if sender_plat in self.ban_list['available_platforms'] and self.ban_list['prohibits'][sender_plat].get(sender_id, None) == None :
            self.ban_list['prohibits'][sender_plat][sender_id] = []
        self.ban_list['prohibits'][sender_plat][sender_id].append(msg_str)

        self.write_ban(self.ban_list)

        speak_prompt_str = (await self.context.persona_manager.get_persona(self.config['speak_prompt'])).system_prompt
        msg = [
        ...
        try :
            speak_res = await self.context.llm_generate(
                chat_provider_id= self.config['speak_config'],
                contexts= msg
            )
        except Exception as e :
            error_msg = traceback.format_exc()
            logger.error(error_msg)
            return
        res_str = self.config['speak_start'] + speak_res.completion_text + self.config['speak_end']
        chain = MessageChain().message(res_str)
        await event.send(chain)
        if self.config['debug_mode'] :
            chain = MessageChain().message(f"[DEBUG]reasoning content:{filter_resoning_res}")
            await event.send(chain)
        event.stop_event()
